...
const slides = [
  ...
  { src: "/img/new/photo_18_2026-03-03_11-10-37.jpg", alt: "Circle Group program participants", label: "Pathways shaped by people" },
  { src: "/img/new/photo_70_2026-03-03_11-10-37.jpg", alt: "Circle Group program participants connecting", label: "Progress made together" },
];
...
export default function HeroCarousel() {
  ...
  return (
    <div className="relative aspect-[4/5] w-full overflow-hidden rounded-[2rem] bg-ink" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-ink/65 via-transparent to-amber/10" />
      ...
      <p aria-live="polite" className="absolute bottom-7 left-7 right-7 font-display text-xl text-white md:bottom-14 md:left-60">{slides[current].label}</p>
      <div className="absolute right-5 top-5 flex gap-2"><button type="button" aria-label="Previous image" onClick={() => go(-1)} className="grid h-9 w-9 place-items-center rounded-full border border-white/60 bg-ink/30 text-white transition hover:bg-forest hover:text-white">←</button><button type="button" aria-label="Next image" onClick={() => go(1)} className="grid h-9 w-9 place-items-center rounded-full border border-white/60 bg-ink/30 text-white transition hover:bg-forest hover:text-white">→</button></div>
