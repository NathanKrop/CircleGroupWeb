...
export default function VoiceCarousel() {
  ...
  return (
    <div className="mx-auto max-w-3xl text-center" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>
      ...
      <div className="min-h-52" aria-live="polite"><AnimatePresence mode="wait"><motion.div key={current} initial={reduceMotion ? false : { opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} exit={reduceMotion ? undefined : { opacity: 0, y: -14 }} transition={{ duration: 0.4 }}><p className="text-balance font-display text-3xl font-medium leading-snug text-ink sm:text-4xl">{voices[current].quote}</p><p className="mt-8 eyebrow text-ink/60">{voices[current].source}</p></motion.div></AnimatePresence></div>
      <div className="mt-6 flex items-center justify-center gap-3"><button type="button" aria-label="Previous voice" onClick={() => go(-1)} className="text-ink/70 transition hover:text-forest">←</button>{voices.map((voice, index) => <button type="button" key={voice.source} aria-label={`Show voice ${index + 1}`} aria-current={current === index} onClick={() => setCurrent(index)} className={`h-2 rounded-full transition-all ${current === index ? "w-6 bg-leaf" : "w-2 bg-leaf/25 hover:bg-forest/55"}`} />)}<button type="button" aria-label="Next voice" onClick={() => go(1)} className="text-ink/70 transition hover:text-forest">→</button></div>
    </div>
  );
}
